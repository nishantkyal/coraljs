...
function SMTPServerConnection(server, client){
    ...
    if (this.server.options.maxClients && this.server.connectedClients > this.server.options.maxClients) {
        ...
        this.client.composeAndSend("220 "+this.server.options.name + " ESMTP " + (this.server.options.SMTPBanner || "node.js simplesmtp"));
    }
}
...
SMTPServerConnection.prototype.end = function(message){
    if(message){
        this.client.composeAndSend(message);
    }
    this.client.end();
};
    ...
    switch((command || "").toString().trim().toUpperCase()){
        ...
        case "EHLO":
            if(!this.server.options.disableEHLO){
                ...
                this.client.composeAndSend("502 5.5.2 Error: command not recognized");
            }
            break;
        ...
        case "VRFY":
            this.client.composeAndSend("252 2.1.5 Send some mail, I'll try my best");
            break;
...
SMTPServerConnection.prototype._onCommandMAIL = function(mail){
    ...
    if(!this.hostNameAppearsAs){
        return this.client.composeAndSend("503 5.5.1 Error: send HELO/EHLO first");
    }

    if(this.server.options.requireAuthentication && !this.authentication.authenticated){
        return this.client.composeAndSend("530 5.5.1 Authentication Required");
    }

    if(this.envelope.from){
        return this.client.composeAndSend("503 5.5.1 Error: nested MAIL command");
    }

    if(!(match = mail.match(/^from\:\s*<([^@>]+\@([^@>]+))>(\s|$)/i))){
        return this.client.composeAndSend("501 5.1.7 Bad sender address syntax");
    }
    ...
    this._validateAddress("sender", email, domain, function (err) {
        if (err) {
            return self.client.composeAndSend(err.message);
        }
        email = email.substr(0, email.length - domain.length) + domain;
        self.envelope.from = email;
        self.client.composeAndSend("250 2.1.0 Ok");
    });
};
...
SMTPServerConnection.prototype._onCommandRCPT = function(mail){
    ...
    if(!this.envelope.from){
        return this.client.composeAndSend("503 5.5.1 Error: need MAIL command");
    }

    if(!(match = mail.match(/^to\:\s*<([^@>]+\@([^@>]+))>$/i))){
        return this.client.composeAndSend("501 5.1.7 Bad recipient address syntax");
    }
...
SMTPServerConnection.prototype._onCommandDATA = function(){
    ...
    if(!this.envelope.to.length){
        return this.client.composeAndSend("503 5.5.1 Error: need RCPT command");
    }

    this.client.startDataMode();
    this.client.composeAndSend("354 End data with <CR><LF>.<CR><LF>");
...
SMTPServerConnection.prototype._onCommandRSET = function(){
    this.init();
    this.client.composeAndSend("250 2.0.0 Ok");
};
...
SMTPServerConnection.prototype._onCommandAUTH = function(payload){
    ...
    if(!this.server.options.requireAuthentication && !this.server.options.enableAuthentication){
        return this.client.composeAndSend("503 5.5.1 Error: authentication not enabled");
    }

    if(!this.server.options.ignoreTLS && !this.client.secureConnection){
        return this.client.composeAndSend("530 5.7.0 Must issue a STARTTLS command first");
    }

    if(this.authentication.authenticated){
        return this.client.composeAndSend("503 5.7.0 No identity changes permitted");
    }
    ...
    if(this.server.options.authMethods.indexOf(method)<0){
        return this.client.composeAndSend("535 5.7.8 Error: authentication failed: no mechanism available");
    }
...
SMTPServerConnection.prototype._onCommandSTARTTLS = function(){
    if(this.client.secureConnection){
        return this.client.composeAndSend("554 5.5.1 Error: TLS already active");
    }

    this.client.composeAndSend("220 2.0.0 Ready to start TLS");
...
SMTPServerConnection.prototype._onCommandHELO = function(host){
    if(!host){
        return this.client.composeAndSend("501 Syntax: EHLO hostname");
    }else{
        this.hostNameAppearsAs = host;
        this.envelope.host = host;
    }
    this.client.composeAndSend("250 "+this.server.options.name+" at your service, ["+
...
SMTPServerConnection.prototype._onCommandEHLO = function(host){
    ...
    this.client.composeAndSend(response.map(function(feature, i, arr){
...
SMTPServerConnection.prototype._onCommandNOOP = function(){
    this.client.composeAndSend("250 OK");
}
...
SMTPServerConnection.prototype._handleAuthPlain = function(payload){
    if (payload.length) {
        ...
        if (userdata.length != 3) {
            return this.client.composeAndSend("500 5.5.2 Error: invalid userdata to decode");
        }
        ...
        this.server.emit("authorizeUser",
            ...
            (function(err, success){
                if(err || !success){
                    ...
                    return this.client.composeAndSend("535 5.7.8 Error: authentication failed: generic failure");
                }
                this.client.composeAndSend("235 2.7.0 Authentication successful");
        ...
        if(this.authentication.state == "NORMAL"){
            ...
            this.client.composeAndSend("334");
        }
    }
...
SMTPServerConnection.prototype._handleAuthLogin = function(payload){
    if(this.authentication.state == "NORMAL"){
        this.authentication.state = "AUTHENTICATING";
        this.client.composeAndSend("334 VXNlcm5hbWU6");
        ...
        if(this.authentication.username === false){
            ...
            this.client.composeAndSend("334 UGFzc3dvcmQ6");
        }else{
...
SMTPServerConnection.prototype._onDataReady = function(){
    if (this.server.listeners('dataReady').length) {
        this.server.emit("dataReady", this.envelope, (function(err, code){
            ...
            if(err){
                this.client.composeAndSend(err && err.SMTPResponse || ("550 " + (err && err.message || "FAILED")));
            }else{
                this.client.composeAndSend("250 2.0.0 Ok: queued as "+(code || crypto.randomBytes(10).toString("hex")));
            }

        }).bind(this));
    } else {
        this.init(true); //reset state, keep auth data
        this.client.composeAndSend("250 2.0.0 Ok: queued as " + crypto.randomBytes(10).toString("hex"));
    }
};
