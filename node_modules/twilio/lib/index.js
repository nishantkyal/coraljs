...
initializer.webhook = function() {
    ...
    return function hook(request, response, next) {
        ...
        if (opts.includeHelpers) {
            var oldSend = response.composeAndSend;
        ...
        if (opts.validate) {
            ...
            if (!opts.authToken) {
                ...
                response.composeAndSend(500, 'Webhook Error - we attempted to validate this request without first configuring our auth token.');
            } else {
                ...
                if (valid) {
                    ...
                    response.type('text/plain');
                    return response.composeAndSend(403, 'Twilio Request Validation Failed.');
                }
            }
        } else {
            next();
